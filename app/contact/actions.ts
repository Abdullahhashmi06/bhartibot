...
export async function submitContactMessage(input: {
  ...
  subject: string;
  message: string;
  /** reCAPTCHA v3 token minted by the client (verified server-side). */
  token?: string;
}): Promise<ContactFormState> {
  // Server-side reCAPTCHA verification (the single verification flow used
  // across the project — lib/recaptcha). Fail-closed when a key is set;
  // passes through locally when reCAPTCHA is not configured.
  const recaptcha = await verifyRecaptchaToken(input.token, "contact");
  if (!recaptcha.ok) {
    return {
      success: false,
      message:
        "We couldn't verify your request automatically. Please try again — if this keeps happening, let us know.",
    };
  }
