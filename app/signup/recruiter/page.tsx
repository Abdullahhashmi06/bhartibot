...
export default function RecruiterSignupPage() {
  ...
  async function handleGoogleSignup() {
    ...
    const check = await verifyRecaptcha("oauth_signup");
    if (!check.ok) {
      setStatus("idle");
      setError(recaptchaErrorMessage());
      return;
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            {step === "details" ? (
              <div className="space-y-5">
                {/* GOOGLE SIGN-IN BUTTON — same flow as recruiter login */}
                <button
                  type="button"
                  onClick={handleGoogleSignup}
                  disabled={status === "loading"}
                  className="flex w-full items-center justify-center gap-3 rounded-xl border border-border bg-slate-50 py-2.5 px-4 text-xs sm:text-sm font-semibold text-text-primary hover:bg-slate-100 hover:shadow-subtle transition-colors shadow-subtle disabled:opacity-50"
                >
